...
class GitLab():
        ...
        while True:
            try:
                ...
                raise ReportError(
                    ...
                    ConnectionResetError) as connection_error:
                retries += 1
    ...
    def search(self,
        ...
        if self.user is None:
            ...
            log.warning("User: %s" % pretty(self.user))
        if self.events is None:
            self.events = self.user_events(self.user['id'], since, until)
            log.warning("Events: %s" % pretty(self.events))
        result = []
        for event in self.events:
            ...
            if (
                ...
                result.append(event)
        log.warning("Result: %s fetched", listed(len(result), "item"))
        return result
...
class Issue():
    ...
    def __str__(self) -> str:
        ...
        if getattr(self.parent.options, 'full_message', False) and self._body:
            body_text = self._body.strip()
    ...
    def note_iid(self, data: dict[str, Any], gitlabapi: GitLab) -> str:
        ...
        if data['note']['noteable_type'] == 'MergeRequest':
            try:
                ...
                log.warning(
                    "Error getting merge request: %s. "
                    "May be due to limiting pagination to %d pages.",
                    e, GITLAB_MAX_PAGE_LIST)
                return "unknown"
            if merge_request is not None:
...
class IssuesCommented(GitLabStats):
    ...
    def fetch(self) -> None:
        log.warning("Searching for Issues commented by %s", self.user)

        # need to add also type DiscussionNote
        results = self.parent.gitlab.search(
            self.user.login, self.options.since, self.options.until,
            target_type='Note',
        ...
        for issue in results:
            log.warning("ISSUE: %s" % pretty(issue))
            log.warning("ISSUE type: %s" % pretty(issue['note']['noteable_type']))

...
ssl_verify = False
""")
    with pytest.raises(ReportError, match=r"Confluence authentication failed"):
...
url = https://localhost
""")
    with pytest.raises(ReportError, match=r"Failed to connect to Confluence"):
        did.cli.main("today")
...
""")
    with pytest.raises(
            ReportError, match=r"The `token` or `token_file` key must be set"
            ):
        ConfluenceStats("confluence")
